...
<div>
  ...
  <img src="https://img.shields.io/badge/argoCD-2D72D2?style=for-the-badge&logo=argo&logoColor=white">
...
# 7. 시스템 아키텍쳐
<img src="https://github.com/user-attachments/assets/8410bc6b-e0f5-4699-8a25-b9fadb4fdcaf" width="1500"/> <br>
# 8. WBS
...
|   조원 이름	|  회고  	|
|---	|---	|
|   차경태 	|도커와 쿠버네티스를 이용한 CI/CD를 처음 공부해봐서 많은 어려움이 있었지만, 끝까지 포기하지 않은 결과 좋은 성과를 낼 수 있었던 것 같습니다. 도메인과 클라우드 컴퓨팅을 이용해 외부접속을 상시 허용하고 부하분산도 적용해보고 싶었지만 시간적, 경제적인 이유로 이번 프로젝트는 여기서 마무리하여서 아쉬움이 남습니다. 그래서 최종 프로젝트에서는 이 아쉬운 점들을 보완해 더욱 성공적인 프로젝트를 하고 싶습니다.     |
|   이창신 	|간단한 시스템 구현 + 배포인데 혼자서 강의를 더듬어가며 해봐도 막히는 게 많았다. 결국 혼자 배포하는 데는 실패했지만 조원들의 도움 덕분에 프로젝트가 잘 마무리될 수 있었다. 최종 프로젝트 때 좀 더 보완해서 데브옵스 파트에도 기여할 수 있는 사람이 되고 싶다.|
|   홍도현 	| 저희가 만든 API를 직접 도커 이미지로 생성해서 쿠버네티스를 통해 관리해보며 도커와 쿠버네티스에 대해 이해할 수 있었습니다. 또한 젠킨스와 깃허브를 연동하며 파이프라인을 통해서 빌드를 자동화하는 것을 구현해보며 CI에 대해 익힐 수 있었습니다. 짧은 시간동안 요구사항 명세부터 배포까지 API개발의 전체적인 프로세스를 구현해보며, 실제 운영환경에서의 프로젝트를 경험할 수 있었습니다. |
|   임현조 	| 쿠버네티스의 여러 오브젝트들을 이용한 배포 설정을 하면서 배포에 대한 흐름을 파악하고 이해도를 높일 수 있었습니다  	|
|   이덕찬	| 수동 배포에서 벗어나 자동화된 CI/CD 환경을 구축하며, 조금은 배포 흐름을 몸소 체득할 수 있었습니다.|
